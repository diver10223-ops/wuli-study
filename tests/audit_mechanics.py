...
def main():
    ...
    for stage in ["diagnostic", "learning", "models", "check", "exam", "retest"]:
        page_name = "m2-a.html" if stage == "learning" else f"m2-a-{stage}.html"
        data_name = f"m2-a-{stage}.js"
        page_path, data_path = ROOT / page_name, ROOT / "data" / data_name
        if not page_path.exists() or not data_path.exists():
            raise SystemExit(f"M2 A: missing {stage} page or data")
    ...
    print("OK: mechanics standard, aggregate progress, M2 A six-stage content and source routes")
