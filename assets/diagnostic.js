(function (global) {
  ...
  function runDiagnostic(config) {
    ...
    function grade() {
      ...
      const passed = eligible && correct >= config.passCorrect && gatesPassed;
      const data = {
      ...
      result.hidden = false;
      result.innerHTML = `<h2 class="${passed ? 'pass' : 'fail'}">${correct}/${questions.length} · ${passed ? config.passTitle : config.failTitle}</h2>
