(function (global) {
  ...
  function runLearning(config) {
    ...
    const admitted = config.admission ? config.admission(storage) : true;
    ...
    if (!admitted) inputs.forEach((input) => { input.disabled = true; });
    if (route && !admitted) route.innerHTML = config.admissionMessage || '<strong>预览模式：</strong>准入证据未齐，不能记录正式进度。';
    function save() {
      if (!admitted) {
        count.textContent = '0'; bar.max = inputs.length; bar.value = 0;
        plan.innerHTML = config.admissionMessage || '<strong>下一步：</strong>先完成准入要求。';
        return;
      }
  ...
  function runExisting(config) {
    const storage = global.WuliStorage;
    if (!storage) throw new Error('WuliStorage must load before learning.js');
    const container = document.querySelector(config.container || '#cards');
    const count = document.querySelector(config.count || '#count');
    const bar = document.querySelector(config.bar || '#bar');
    const plan = document.querySelector(config.plan || '#plan');
    if (!container || !count || !bar || !plan) throw new Error('Existing learning page is missing a required element');
    const diagnosis = config.diagnosticKey ? storage.read(config.diagnosticKey, null) : null;
    const admitted = config.admission ? config.admission(storage) : true;
    const saved = storage.read(config.storageKey, {});
    const done = new Set(saved.done || []);
    const inputs = [...container.querySelectorAll(config.inputSelector || '.task input[type="checkbox"]')];
    inputs.forEach((input) => { input.checked = done.has(input.value); input.disabled = !admitted; });
    (diagnosis?.weak || []).forEach((node) => container.querySelector(`[data-node="${node}"]`)?.classList.add('priority-card'));
    function save() {
      const completed = admitted ? inputs.filter((input) => input.checked).map((input) => input.value) : [];
      if (admitted) storage.write(config.storageKey, { version: 2, module: config.module, track: config.track, done: completed, updatedAt: new Date().toISOString() });
      count.textContent = String(completed.length); bar.max = inputs.length; bar.value = completed.length;
      plan.innerHTML = admitted
        ? completed.length === inputs.length ? config.completePlan : `<strong>当前：</strong>还剩${inputs.length - completed.length}项。`
        : config.admissionMessage;
    }
    inputs.forEach((input) => input.addEventListener('change', save)); save();
    return Object.freeze({ save, inputs });
  }

  global.WuliLearning = Object.freeze({ run: runLearning, runExisting });
})(window);
